...
export function SignupForm({ slug }: { slug: string | null | undefined }) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const redirectTo = slugNorm
      ? `${typeof window !== "undefined" ? window.location.origin : ""}/onboarding?s=${encodeURIComponent(slugNorm)}`
    ...
    router.push(slugNorm ? `/onboarding?s=${encodeURIComponent(slugNorm)}` : "/onboarding");
  };
    ...
    <main className="flex min-h-[calc(100vh-64px)] items-center justify-center px-4 bg-slate-50">
      <div className="w-full max-w-md space-y-6 rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="text-xs text-slate-600">
          ...
          <button
            type="button"
            onClick={() => router.push(buildAuthPath("/login", slugNorm))}
