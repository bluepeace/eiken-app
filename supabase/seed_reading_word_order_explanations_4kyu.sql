...
UPDATE public.reading_word_order_questions SET explanation = 'Paul didn''t have time to eat breakfast. で「ポールは朝食を食べる時間がなかった」。to eat breakfast は to不定詞の目的用法です。' WHERE level = '4級' AND prompt_ja = 'ポールは今日遅く起きたので、朝食を食べる時間がありませんでした。';
...
UPDATE public.reading_word_order_questions SET explanation = 'Carol left her house at 7 for school. leave 〜 for 〜 で「〜を出て〜へ向かう」。at 7 は「7時に」の意味です。' WHERE level = '4級' AND prompt_ja = 'キャロルは昨日7時に家を出て学校へ向かいました。';
...
UPDATE public.reading_word_order_questions SET explanation = 'She had a great weekend in Kyoto last week. 主語 + 動詞 + 目的語 + 場所 + 時の語順。have a great weekend で「楽しい週末を過ごす」。' WHERE level = '4級' AND prompt_ja = '彼女は先週京都で楽しい週末を過ごしました。';
...
-- ========== 追加問題（レベルチェック４級例題） ==========
UPDATE public.reading_word_order_questions SET explanation = 'The classroom was very quiet during the math test. 主語 + be動詞 + 補語 + 時の語順。during 〜 で「〜の間」を表します。' WHERE level = '4級' AND prompt_ja = '数学のテストの間、教室はとても静かでした。';
