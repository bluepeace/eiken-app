...
sub init {
	...
	$self->{csv_cols} = [
		["cpnacts.cpnact_id",        "入出金識別ID"],
		["cpnacts.coupon_id",     "クーポン識別ID"],
		["cpnacts.member_id",        "$self->{conf}->{member_caption}識別ID"],
		["members.member_lastname",  "姓"],
		["members.member_firstname", "名"],
		["members.member_handle",    "ニックネーム"],
		["cpnacts.seller_id",        "代理店識別ID"],
		["sellers.seller_company",   "代理店会社名"],
		["cpnacts.cpnact_type",      "入出金種別", { "1" => "入金", "2" => "出金" }],
		["cpnacts.cpnact_reason",    "入出金摘要", 0, $cpnact_reason_cap],
		["cpnacts.cpnact_cdate",     "入出金日時", 1],
		["cpnacts.cpnact_price",     "金額"],
		["cpnacts.lsn_id",           "注文識別ID"]
	];
}
...
sub charge {
	...
	if($p->{cpnact_type} == 1) {
		...
		$sql3 = "UPDATE members SET member_coupon_expire=${expire_date} WHERE member_id=$p->{member_id}";
	}
	#SQL実行
	my $last_sql;
	my $cpnact_id;
...
sub get_csv {
	...
	my @list;
	{
		my $sql = "SELECT " . join(",", @col_list) . " FROM cpnacts";
