...
sub init {
    ...
    $self->{table_cols} = {
		...
		prof_bank_code => '金融機関コード',
		prof_bank_name => '金融機関名',
		prof_bank_branch_code => '支店コード',
		prof_bank_branch_name => '支店名',
		prof_bank_account_type => '口座種別',
		prof_bank_account_number => '口座番号',
		prof_bank_account_name => '口座名義',
		prof_deduction => '源泉徴収フラグ',
		prof_invoice_registration => '課税事業者登録番号'
    };
    ...
    $self->{csv_cols} = [
		...
		['prof_bank_branch_code', '支店コード'],
		['prof_bank_branch_name', '支店名'],
		['prof_bank_account_type', '口座種別 (1:普通、2:当座、4:貯蓄)'],
		['prof_bank_account_number', '口座番号'],
		['prof_bank_account_name', '口座名義'],
		['prof_deduction', '源泉徴収フラグ'],
		['prof_invoice_registration', '課税事業者登録番号']
    ...
    for my $k ( @{$names} ) {
        ...
        #パスワード
            ...
            if ( $v eq "" ) {
                ...
                push( @errs, [ $k, "\"${caption}\" は8文字以上100文字以内で入力してください。" ] );
            }
            elsif ( $v =~ /[^\x21-\x7e]/ ) {
        ...
        #役職
            ...
            if ( $v eq "" ) {
                ...
                my $chkref = $self->get_from_db_by_title($v);
                if ( $mode eq "mod" ) {    #修正時
                    my $me = $self->get_from_db( $in->{prof_id} );
                    if ( $v ne $me->{prof_title} && defined $chkref && $chkref ) {
                        push( @errs, [ $k, "\"${caption}\" はすでに登録されています。" ] );
                    }
                }
                else {                     #新規登録時
		...
		#金融機関コード
		elsif($k eq "prof_bank_code") {
			if($v eq "") {

			} elsif($v !~ /^\d{4}$/) {
				push(@errs, [$k, "\"${caption}\" は半角数字4桁で入力してください。"]);
			}
		} 

		#金融機関名
        elsif($k eq "prof_bank_name") {
			if($v eq "") {
		...
		#支店コード
		elsif($k eq "prof_bank_branch_code") {
			if($v eq "") {

			} elsif($v !~ /^\d{3}$/) {
				push(@errs, [$k, "\"${caption}\" は半角数字3桁で入力してください。"]);
			}
        } 
		#支店名
		elsif($k eq "prof_bank_branch_name") {
			if($v eq "") {
		...
		#口座種別
		elsif($k eq "prof_bank_account_type") {
			if($v eq "") {
		...
		#口座番号
		elsif($k eq "prof_bank_account_number") {
			if($v eq "") {

			} elsif($v !~ /^\d{7}$/) {
				push(@errs, [$k, "\"${caption}\" は半角数字7桁で入力してください。"]);
			}
        } 
		#口座名義
		elsif($k eq "prof_bank_account_name") {
			if($v eq "") {

			} else {
				my $obankchar = new FCC::Class::Bankchar();
				my $err = $obankchar->check_bank_account_name($v, $caption);
				if($err) {
					push(@errs, [$k, $err]);
				} else {
					my $han = $obankchar->conv_bank_account_name_han($v);
					my $han_len = FCC::Class::String::Checker->new($han, "utf8")->get_char_num();
					if($han_len > 48) {
						push(@errs, [$k, "\"${caption}\" は48文字以内で入力してください。"]);
					}
				}
			}
        } 
		#源泉徴収フラグ
		elsif($k eq "prof_deduction") {
			unless($v) { $v = 0; }
			if($v !~ /^(0|1)$/) {
				push(@errs, [$k, "\"${caption}\" に不正な値が送信されました。"]);
			} else {
		...
		#口座番号
		elsif($k eq "prof_invoice_registration") {
			if($v eq "") {

			} elsif($v !~ /^[a-zA-Z0-9]{14}$/) {
				push(@errs, [$k, "\"${caption}\" は半角英数字14桁で入力してください。"]);
			}
        } 
...
#	全設定情報を格納したhashrefを返す。
#---------------------------------------------------------------------
sub get_from_db_by_title {
    my ( $self, $prof_title ) = @_;
    if ( !defined $prof_title || $prof_title eq "" ) {
        croak "the 1st argument is invaiid.";
    }
    #
    return $self->_get_from_db( "prof_title", $prof_title );
}
