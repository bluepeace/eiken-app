...
sub init {
    ...
    $self->{table_cols} = {
        ...
        lsn_prof_price_zei         => "講師への支払金額（税込）"
    };
    ...
    $self->{csv_cols} = [
        ...
        [ "lessons.lsn_prof_price_zei",    "講師への支払金額（税込）" ],
    ...
    $self->{csvcus_cols} = [
        [ "lessons.lsn_id",           "レッスン識別ID" ],
        [ "courses.course_name",      "レッスン種類" ],
        [ "lessons.prof_id",          "講師ID" ],
        [ "profs.prof_handle",        "講師名" ],
        [ "lessons.member_id",        "会員ID" ],
        [ "members.member_handle",    "会員名" ],
        [ "lessons.lsn_stime",         "レッスン日時" ],
        [ "lessons.lsn_prof_fee",    "費用（pt）" ],
        [ "lessons.lsn_pay_type", "通常・無料体験", 0, { "1" => "ポイント", "2" => "クーポン" } ],
        [ "lessons.coupon_id",         "クーポン識別ID" ],
        [ "lessons.lsn_pay_fee_rate",  "配分ベース比率" ],
        [ "lessons.lsn_prof_price",    "講師報酬（税抜）計上数値" ],
        [ "lessons.lsn_prof_price_zei",    "講師報酬（税込）支払通知書記載" ],
        [ "members.member_st_flag",   "会員の種類", 0, { "1" => "体験", "2" => "有料", "3" => "モニター", "4" => "キャンペーン" } ]
    ];
    #
    $self->{now} = time;
...
sub update_status {
    ...
    if ( $lsn_status > 0 && $lsn_status < 20 ) {
        ...
        $rec->{lsn_prof_price_zei}   = int( $rec->{lsn_base_price} * $rec->{lsn_prof_margin} / 100 * 1.1 );
    }
...
sub get_csvcus {
    ...
    my @col_list;
    my @col_name_list;
    for ( my $i = 0 ; $i < @{ $self->{csvcus_cols} } ; $i++ ) {
        my $r = $self->{csvcus_cols}->[$i];
        push( @col_list,      $r->[0] );
        push( @col_name_list, $r->[1] );
    }
    ...
    my @list;
    {
        ...
        while ( my $ref = $sth->fetchrow_arrayref ) {
            for ( my $i = 0 ; $i < @{$ref} ; $i++ ) {
                ...
                if ( $self->{csvcus_cols}->[$i]->[2] && $ref->[$i] ) {
                    my @tm = FCC::Class::Date::Utils->new( time => $ref->[$i], tz => $self->{conf}->{tz} )->get(1);
                    $ref->[$i] = "$tm[0]-$tm[1]-$tm[2] $tm[3]:$tm[4]:$tm[5]";
                }
                elsif ( $self->{csvcus_cols}->[$i]->[3] && $ref->[$i] ne "" ) {
                    my $cap = $self->{csvcus_cols}->[$i]->[3]->{ $ref->[$i] };
                    if ($cap) {
                        $ref->[$i] = $cap;
                    }
                }
...
sub get_list {
    ...
    #指定の検索条件を新たなhashrefに格納
    my $params         = {};
    my @param_key_list = ( 'lsn_id', 'prof_id', 'member_id', 'seller_id', 'course_id', 'pdm_id', 'lsn_status', 'lsn_cancel', 'lsn_status_date_s', 'lsn_status_date_e', 'lsn_stime_s', 'lsn_stime_e', 'lsn_etime_s', 'lsn_etime_e', 'offset', 'limit', 'sort', 'lsn_status_list',);
    if ( defined $in_params ) {
        for my $k (@param_key_list) {
            if ( defined $in_params->{$k} && $in_params->{$k} ne "" ) {
                $params->{$k} = $in_params->{$k};
	...
	if(defined $params->{lsn_status_list}) {
		my $lsn_status_in = join(", ", @{$params->{lsn_status_list}});
		push(@wheres, "lessons.lsn_status IN (${lsn_status_in})");
	}
...
sub pay_back {
    ...
    eval {
        ...
        #　・lsn_status=31 (払い戻し)
        #--------------------------------------------------------------
        $last_sql = "UPDATE lessons SET lsn_base_price=0, lsn_prof_price=0, lsn_seller_price=0, lsn_status=31, lsn_prof_price_zei=0 WHERE lsn_id=${lsn_id}";
        $dbh->do($last_sql);
