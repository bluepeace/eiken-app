...
sub update_all {
	...
	$self->get_prof_toeic_score_from_db();
	$self->get_prof_eiken1_score_from_db();
	$self->get_prof_eiken2_from_db();
	#
	return 1;
}
...
sub get_prof_eiken1_score {
	my($self) = @_;
	my $ref = $self->get_from_memcache("prof_eiken1_score");
	unless($ref && ref($ref) eq "ARRAY") {
		$ref = $self->get_prof_eiken1_score_from_db();
	}
	return $ref;
}

sub get_prof_eiken1_score_from_db {
	my($self) = @_;
	#講師情報を検索
	my $oprof = new FCC::Class::Prof(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $params = {
	...
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("prof_eiken1_score", $list);
	return $list;
}

sub get_prof_eiken2_score {
	my($self) = @_;
	my $ref = $self->get_from_memcache("prof_eiken2_score");
	unless($ref && ref($ref) eq "ARRAY") {
		$ref = $self->get_prof_eiken2_score_from_db();
	}
	return $ref;
}

sub get_prof_eiken2_score_from_db {
	my($self) = @_;
	#講師情報を検索
	my $oprof = new FCC::Class::Prof(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $params = {
	...
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("prof_eiken2_score", $list);
	return $list;
}

sub get_prof_toeic_score {
	my($self) = @_;
	my $ref = $self->get_from_memcache("prof_toeic_score");
	unless($ref && ref($ref) eq "ARRAY") {
		$ref = $self->get_prof_toeic_score_from_db();
	}
	return $ref;
}

sub get_prof_toeic_score_from_db {
	my($self) = @_;
	#講師情報を検索
	my $oprof = new FCC::Class::Prof(conf=>$self->{conf}, db=>$self->{db}, memd=>$self->{memd});
	my $params = {
	...
	my $list = $res->{list};
	#memcacheにセット
	$self->set_to_memcache("prof_toeic_score", $list);
	return $list;
}
