...
sub check_date_range {
    my ( $self, $sdate, $edate ) = @_;
    my ( $self, $stdate, $etdate ) = @_;

    my ( $sY, $sM, $sD ) = $sdate =~ /^(\d{4})\-(\d{2})\-(\d{2})$/;
    my ( $eY, $eM, $eD ) = $edate =~ /^(\d{4})\-(\d{2})\-(\d{2})$/;
    my ( $stY, $stM, $stD ) = $stdate =~ /^(\d{4})\-(\d{2})\-(\d{2})$/;
    my ( $etY, $etM, $etD ) = $etdate =~ /^(\d{4})\-(\d{2})\-(\d{2})$/;
    ...
    unless ( Date::Pcalc::check_date( $stY, $stM, $stD ) ) {
        croak "The `stdate` is invalid: ${stdate}";
    }
    unless ( Date::Pcalc::check_date( $etY, $etM, $etD ) ) {
        croak "The `etdate` is invalid: ${etdate}";
    }
    ...
    my $stepoch = FCC::Class::Date::Utils->new(
        iso => "${stY}-${stM}-${stD} 00:00:00",
        tz  => $tz
    )->epoch();

    my $etepoch = FCC::Class::Date::Utils->new(
        iso => "${etY}-${etM}-${etD} 23:59:59",
        tz  => $tz
    )->epoch();

    return ( $sepoch, $eepoch, $stepoch, $etepoch );
}
...
sub csv11 {
    ...
    $sql .= " FROM mbracts";
    $sql .= " LEFT JOIN members ON mbracts.member_id=members.member_id";
    $sql .= " WHERE mbracts.mbract_reason=61";
    $sql .= " AND mbracts.mbract_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count12 {
    ...
    my $sql = "SELECT COUNT(*) FROM cpnacts";
    $sql .= " WHERE cpnact_reason=11";
    $sql .= " AND cpnact_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count13 {
    ...
    my $sql = "SELECT COUNT(*) FROM mbracts";
    $sql .= " WHERE mbract_reason=61";
    $sql .= " AND mbract_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count14 {
    ...
    my $sql = "SELECT COUNT(*) FROM lessons";
    $sql .= " WHERE lsn_status IN (1, 12, 13)";
    $sql .= " AND lsn_status_date BETWEEN ${sepoch} AND ${eepoch}";
...
#---------------------------------------------------------------------
#■レッスン管理 lesson の「完了」「会員緊急キャンセル」「会員放置キャンセル」の各レコード合算金額（クーポンのみ）
#---------------------------------------------------------------------
#[引数]
#	1. 集計開始日 (YYYY-MM-DD)
#	2. 集計終了日 (YYYY-MM-DD)
...
sub count16 {
    ...
    my $sql = "SELECT COUNT(*) FROM cards";
    $sql .= " WHERE crd_success=1";
    $sql .= " AND crd_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count18 {
    ...
    my $sql = "SELECT COUNT(*) FROM profs";
    $sql .= " WHERE prof_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count19 {
    ...
    $sql .= " WHERE member_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count20 {
    ...
    my $sql = "SELECT COUNT(*) FROM members";
    $sql .= " WHERE member_st_flag=3";
    $sql .= " AND member_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count21 {
    ...
    my $sql = "SELECT COUNT(DISTINCT member_id) FROM lessons";
    $sql .= " WHERE lsn_stime BETWEEN ${q_sdt} AND ${q_edt}";
...
sub count22 {
    ...
    my $sql = "SELECT COUNT(DISTINCT prof_id) FROM lessons";
    $sql .= " WHERE lsn_stime BETWEEN ${q_sdt} AND ${q_edt}";
...
sub count23 {
    ...
    my $sql = "SELECT SUM(crd_price) FROM cards";
    $sql .= " WHERE crd_success=1";
    $sql .= " AND crd_cdate BETWEEN ${sepoch} AND ${eepoch}";
...
sub count28 {
    ...
    # 引数をチェック
    my ( $stepoch, $etepoch ) = $self->check_date_range( $stdate, $etdate );
    my $dbh = $self->{db}->connect_db();

    my $sql = "SELECT SUM(cpnact_price) FROM cpnacts";
    $sql .= " WHERE cpnact_reason=11";
    $sql .= " AND cpnact_cdate BETWEEN ${stepoch} AND ${etepoch}";
...
sub count30 {
    ...
    my $sql = "SELECT SUM(mbract_price) FROM mbracts";
    $sql .= " WHERE mbract_reason=12";
    $sql .= " AND mbract_cdate BETWEEN ${stepoch} AND ${etepoch}";
