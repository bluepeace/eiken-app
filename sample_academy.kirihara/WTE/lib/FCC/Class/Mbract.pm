...
#	}
	...
	#会員のプラン状況を有料会員へ
	my $sql4;
	if($p->{mbract_reason} == 41) {
		$sql4 = "UPDATE members SET members.member_st_flag=2 WHERE members.member_id=$p->{member_id}";
	}
	...
	if($p->{mbract_reason} == 41) {
		...
		$sql5 = "UPDATE members, plans SET members.member_course_flag=plans.pln_no WHERE members.member_id=$p->{member_id} and plans.pln_id='" . $p->{pln_id} . "'";
	}
