...
sub init {
	...
	$self->{csv_cols} = [
		...
		["cards.crd_success",      "決済完了フラグ", 0, { "1" => "成功", "2" => "失敗" }],
...
sub get_list {
	...
	while( my($k, $v) = each %{$params} ) {
		if($k =~ /^(crd|member|auto)_id$/) {
			...
			$params->{$k} = $v + 0;
		} elsif($k eq "crd_cdate1") {
			if($v =~ /[^\d]/) {
				delete $params->{$k};
			}
		} elsif($k eq "crd_cdate2") {
			if($v =~ /[^\d]/) {
				delete $params->{$k};
			}
		} elsif($k eq "sort") {
	...
	if(defined $params->{crd_cdate1}) {
		push(@wheres, "cards.crd_cdate>=$params->{crd_cdate1}");
	}
	if(defined $params->{crd_cdate2}) {
		push(@wheres, "cards.crd_cdate<=$params->{crd_cdate2}");
	}
	if(defined $params->{crd_txn_id}) {
		my $q_v = $dbh->quote($params->{crd_txn_id});
		push(@wheres, "cards.crd_txn_id=${q_v}");
