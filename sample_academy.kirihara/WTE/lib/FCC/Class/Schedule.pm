...
sub get_list {
	...
	if(defined $params->{lsn_id}) {
		push(@wheres, "schedules.lsn_id=$params->{lsn_id}");
	}
    ...
    if ( defined $params->{sch_stime_s} ) {
        my ( $Y, $M, $D ) = $params->{sch_stime_s} =~ /^(\d{4})(\d{2})(\d{2})/;
        my $h = "00";
        my $m = "00";
        if ( $params->{sch_stime_s} =~ /^\d{8}(\d{2})(\d{2})/ ) {
            $h = $1;
            $m = $2;
        }
        my $qv = $dbh->quote("${Y}-${M}-${D} ${h}:${m}:00");
        push( @wheres, "schedules.sch_stime >= ${qv}" );
    }
    if ( defined $params->{sch_stime_e} ) {
        my ( $Y, $M, $D ) = $params->{sch_stime_e} =~ /^(\d{4})(\d{2})(\d{2})/;
        my $h = "23";
        my $m = "59";
        if ( $params->{sch_stime_e} =~ /^\d{8}(\d{2})(\d{2})/ ) {
            $h = $1;
            $m = $2;
        }
	...
	my @list;
	{
		my $sql = "SELECT schedules.*, profs.* FROM schedules";
		$sql .= " LEFT JOIN profs ON schedules.prof_id=profs.prof_id";
		if(@wheres) {
