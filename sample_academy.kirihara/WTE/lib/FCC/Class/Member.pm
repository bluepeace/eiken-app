...
sub init {
	...
	$self->{table_cols} = {
		...
		member_coupon_expire => $self->{conf}->{member_coupon_expire_caption},
		member_st_flag      => $self->{conf}->{member_st_flag_caption},
		member_course_flag      => $self->{conf}->{member_course_flag_caption}
	};
...
#		}
		...
		if($ref->{member_coupon} > 0) {
			if($ref->{member_coupon_expire} lt $today) {
				$ref->{member_coupon} = 0;
			}
		}
	}
...
sub get_list {
	...
	while( my($k, $v) = each %{$params} ) {
		...
		} elsif($k eq "member_cdate1") {
			if($v =~ /[^\d]/) {
				delete $params->{$k};
			}
		} elsif($k eq "member_cdate2") {
			if($v =~ /[^\d]/) {
				delete $params->{$k};
			}
		} elsif($k eq "member_st_flag") {
	...
	if(defined $params->{member_cdate1}) {
		push(@wheres, "member_cdate>=$params->{member_cdate1}");
	}
	if(defined $params->{member_cdate2}) {
		push(@wheres, "member_cdate<=$params->{member_cdate2}");
	}
	if(defined $params->{member_st_flag}) {
		push(@wheres, "member_st_flag=$params->{member_st_flag}");
	}
	if(defined $params->{member_course_flag}) {
		push(@wheres, "member_course_flag=$params->{member_course_flag}");
	}
	if(defined $params->{member_status}) {
		push(@wheres, "member_status=$params->{member_status}");
	}
...
sub add_multi {
	...
	eval {
		for my $ref (@{$list}) {
			...
			# パスワード
			my $pass_plain = $rec->{member_pass};
			$rec->{member_pass} = FCC::Class::PasswdHash->new()->generate($pass_plain);
			...
			$last_sql = "INSERT INTO members (" . join(",", @klist) . ") VALUES (" . join(",", @vlist) . ")";

			#INSERT
			$dbh->do($last_sql);
			my $member_id = $dbh->{mysql_insertid};
			$rec->{member_pass} = $pass_plain;
			push(@rec_list, $rec)
		}
		$dbh->commit();
	};
...
sub create_pass {
	my($self) = @_;
	my $DIGIT = 12;
	my @seed = ('a'..'z', 'A'..'Z', 0..9, '#', '!', '$', '%', '_', '&', '*');
	my $seed_num = scalar @seed;
	my $pass = '';
	while(length $pass < $DIGIT) {
		$pass .= @seed[int rand($seed_num)];
	}
	return $pass;
}
