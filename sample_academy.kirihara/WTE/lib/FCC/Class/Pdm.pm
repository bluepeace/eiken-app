...
sub init {
    ...
    $self->{table_cols} = {
        ...
        pdm_status => "支払ステータス",
        pdm_price  => "請求金額",
        pdm_price_zei  => "請求金額（税込）",
        pdm_price_gensen  => "源泉徴収金額",
        pdm_price_fix  => "支払金額"
    };
    ...
    $self->{csv_cols} = [
        ...
        [ "pdms.pdm_price",  "請求金額" ],
        [ "pdms.pdm_price_zei",  "請求金額（税込）" ],
        [ "pdms.pdm_price_gensen",  "源泉徴収金額" ],
        [ "pdms.pdm_price_fix",  "支払金額" ]
    ];
...
sub get_demand_target {
    ...
    while ( my $ref = $sth->fetchrow_hashref ) {
        ...
        $pdm_price_zei += $ref->{lsn_prof_price_zei};
    }
    $sth->finish();
    #
    my $res = {
...
sub add {
    ...
    my $pdm_price_zei   = int($demand_target->{pdm_price} * 1.1);
#    my $pdm_price_gensen   = int($demand_target->{pdm_price} * 1.1 * 0.1021);
    my $pdm_price_gensen   = int($demand_target->{pdm_price} * 0.1021);
    my $pdm_price_fix   = $pdm_price_zei - $pdm_price_gensen;
    ...
    my $rec = {
        ...
        pdm_price  => $pdm_price,
        pdm_price_zei  => $pdm_price_zei,
        pdm_price_gensen  => $pdm_price_gensen,
        pdm_price_fix  => $pdm_price_fix
    };
...
sub get_paypay_csv {
	...
	# FCC:Class::Bankcharインスタンス
	my $obankchar = new FCC::Class::Bankchar();
	#振込依頼人名
	my $client_name = $obankchar->conv_bank_account_name_han($self->{conf}->{pdm_paypay_client_name});
    ...
    my @col_list;
    my @col_name_list;
    for ( my $i = 0 ; $i < @{ $self->{psv_cols} } ; $i++ ) {
        my $r = $self->{psv_cols}->[$i];
        push( @col_list,      $r->[0] );
        push( @col_name_list, $r->[1] );
    }
	...
	my @col_list = (
		"pdms.pdm_price",
		"pdms.pdm_price_fix",
		"pdms.pdm_price_gensen",
		"pdms.pdm_cdate",
		"profs.prof_id",
		"profs.prof_lastname",
		"profs.prof_firstname",
		"profs.prof_handle",
		"profs.prof_title",
		"profs.prof_bank_code",
		"profs.prof_bank_name",
		"profs.prof_bank_branch_code",
		"profs.prof_bank_branch_name",
		"profs.prof_bank_account_type",
		"profs.prof_bank_account_number",
		"profs.prof_bank_account_name",
		"profs.prof_deduction"
	);
	...
	$sth->execute();
	my $csv = "";
	my $rcode = $params->{returncode};
	while( my $ref = $sth->fetchrow_hashref ) {
		my $pdm_price = $ref->{pdm_price};
		my $price = $pdm_price;
		if($ref->{prof_deduction}) {
			$price = int($price * 0.8979 + 0.5);
		}
		my $price_zei = int($pdm_price * 1.1);

		my @tm = FCC::Class::Date::Utils->new(time=>$ref->{pdm_cdate}, tz=>$self->{conf}->{tz})->get(1);
		my $cdate = "$tm[0]/$tm[1]/$tm[2] $tm[3]:$tm[4]:$tm[5]";

		my $cols = [
			$ref->{prof_bank_name}, # 銀行名
			"325", # 勘定科目（固定）
			"0", # 補助科目（固定）
			$ref->{prof_title}, # 奉行ID
			$ref->{prof_lastname} . $ref->{prof_firstname}, # 講師名
			$ref->{prof_bank_name}, # 銀行名
			$ref->{prof_bank_branch_name}, # 支店名
			$ref->{prof_bank_code}, # 銀行コード
			$ref->{prof_bank_branch_code}, # 支店コード
			$ref->{prof_bank_account_type}, # 預金種目 (1:普通, 2:当座, 4:貯蓄)
			$ref->{prof_bank_account_number}, # 口座番号
			$obankchar->conv_bank_account_name_han($ref->{prof_bank_account_name}), # 受取人名
			$ref->{prof_deduction}, # 源泉徴収フラグ
			$price_zei, # 消費税込み
			$ref->{pdm_price_gensen}, # 請求金額
			$ref->{pdm_price_fix}, # 請求金額
			$price, # 振込額
			$client_name, # 振込依頼人名
			$ref->{prof_id}, # 講師識別ID
			$cdate # 請求日時
