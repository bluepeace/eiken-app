package FCC::View::Admin::PrfschfrmView;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	for my $ymd (@{$context->{week}}) {
		...
		$hash->{member_id} = $context->{member}->{member_id};
		push(@week_loop, $hash);
	}
	...
	for(my $i=0; $i<@{$res->{list}}; $i+=$cols) {
		...
		for(my $j=$i; $j<$i+$cols; $j++) {
			...
			$th_hash->{epoch} = $epoch;
			$th_hash->{member_id} = $context->{member}->{member_id};
		...
		for my $tm (@{$context->{time_list}}) {
			...
			for my $prof (@{$prof_list}) {
				...
				if($sch_hash && $sch_hash->{$prof_id}) {
					$sch_hash->{$prof_id}->{member_id} = $context->{member}->{member_id};
