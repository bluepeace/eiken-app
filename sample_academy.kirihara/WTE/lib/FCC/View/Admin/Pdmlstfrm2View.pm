package FCC::View::Admin::Pdmlstfrm2View;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	for my $ref (@{$res->{list}}) {
		...
		while( my($k, $v) = each %{$ref} ) {
			...
			} elsif($k =~ /^pdm_(price)_zei$/) {
				$hash{"${k}_with_comma"} = FCC::Class::String::Conv->new($v)->comma_format();
			} elsif($k =~ /^pdm_(price)_gensen$/) {
				$hash{"${k}_with_comma"} = FCC::Class::String::Conv->new($v)->comma_format();
			} elsif($k =~ /^pdm_(price)_fix$/) {
				$hash{"${k}_with_comma"} = FCC::Class::String::Conv->new($v)->comma_format();
			}
		}
	...
	$t->param("paypay_download_url" => $res->{paypay_download_url});
	#
	$self->print_html($t);
}
