...
sub dispatch {
	...
	while( my($k, $v) = each %{$context->{lsn}} ) {
		...
		if($k =~ /^(prof_cdate|prof_mdate)$/) {
			...
			$t->param("${k}_${v}" => 1);
		} elsif($k =~ /^member_(gender|st_flag)$/) {
			$t->param("${k}_${v}" => 1);
			...
			$tmp =~ s/\n/<br \/>/g;
			$t->param($k => $tmp);
		} elsif($k =~ /^member_(intro|comment|memo)$/) {
			my $tmp = CGI::Utils->new()->escapeHtml($v);
			$tmp =~ s/\n/<br \/>/g;
			$t->param($k => $tmp);
	...
	#レッスン履歴
	my $mlsn_num = scalar @{$context->{mlsn_list}};
	$t->param("mlsn_num" => $mlsn_num);
	my @mlsn_loop;
	for my $mlsn (@{$context->{mlsn_list}}) {
		my %h;
		while( my($k, $v) = each %{$mlsn} ) {
			$h{$k} = CGI::Utils->new()->escapeHtml($v);
			if($k eq "mlsn_content") {
				$v = CGI::Utils->new()->escapeHtml($v);
				$v =~ s/(https?\:\/\/[0-9a-zA-Z\:\/\.\-\_\#\%\&\=\~\+\?\;\,]+)/<a href=\"$1\" target=\"_blank\">$1<\/a\>/g;
				$v =~ s/\n/<br \/>/g;
				$h{$k} = $v;
