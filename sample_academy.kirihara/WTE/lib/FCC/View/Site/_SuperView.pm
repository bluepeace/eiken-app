...
sub load_template {
	...
	if( grep(/^pref_eiken1_score_loop$/, @parameter_names) ) {
		my $prof_intro_chars = $params->{pref_eiken1_score_loop}->{PROF_INTRO_CHARS} + 0;
		unless($prof_intro_chars) { $prof_intro_chars = 100; }
		my $limit = $params->{pref_eiken1_score_loop}->{LIMIT} + 0;
		unless($limit) { $limit = 5; }
		#
		my $list = $osp->get_prof_eiken1_score();
		my @loop;
		my $n = 0;
		for my $ref (@{$list}) {
			my $hash = $self->make_prof_list_template_hash($ref, $prof_intro_chars);
		...
		$t->param("pref_eiken1_score_loop" => \@loop);
	}
	if( grep(/^pref_eiken2_score_loop$/, @parameter_names) ) {
		my $prof_intro_chars = $params->{pref_eiken2_score_loop}->{PROF_INTRO_CHARS} + 0;
		unless($prof_intro_chars) { $prof_intro_chars = 100; }
		my $limit = $params->{pref_eiken2_score_loop}->{LIMIT} + 0;
		unless($limit) { $limit = 5; }
		#
		my $list = $osp->get_prof_eiken2_score();
		my @loop;
		my $n = 0;
		for my $ref (@{$list}) {
			my $hash = $self->make_prof_list_template_hash($ref, $prof_intro_chars);
		...
		$t->param("pref_eiken2_score_loop" => \@loop);
	}
	if( grep(/^pref_toeic_score_loop$/, @parameter_names) ) {
		my $prof_intro_chars = $params->{pref_toeic_score_loop}->{PROF_INTRO_CHARS} + 0;
		unless($prof_intro_chars) { $prof_intro_chars = 100; }
		my $limit = $params->{pref_toeic_score_loop}->{LIMIT} + 0;
		unless($limit) { $limit = 5; }
		#
		my $list = $osp->get_prof_toeic_score();
		my @loop;
		my $n = 0;
		for my $ref (@{$list}) {
			my $hash = $self->make_prof_list_template_hash($ref, $prof_intro_chars);
		...
		$t->param("pref_toeic_score_loop" => \@loop);
	}
