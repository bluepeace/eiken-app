package FCC::Action::Admin::Crdlstfrm2Action;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
	...
	while( my($k, $v) = each %{$in} ) {
		...
		if($k eq "cdate1") {
			if($v =~ /^\d{4}\-\d{2}\-\d{2}$/) {
				my $iso = "${v} 00:00:00";
				$params->{"crd_cdate1"} = FCC::Class::Date::Utils->new(time=>$v, tz=>$self->{conf}->{tz}, iso=>$iso)->epoch();
			} else {
				next;
			}
		} elsif($k eq "cdate2") {
			if($v =~ /^\d{4}\-\d{2}\-\d{2}$/) {
				my $iso = "${v} 23:59:59";
				$params->{"crd_cdate2"}  = FCC::Class::Date::Utils->new(time=>$v, tz=>$self->{conf}->{tz}, iso=>$iso)->epoch();
			} else {
				next;
			}
		}
	...
	#
	if($params->{crd_cdate1}) {
		$res->{params}->{cdate1} = $params->{cdate1};
	}
	if($params->{crd_cdate2}) {
