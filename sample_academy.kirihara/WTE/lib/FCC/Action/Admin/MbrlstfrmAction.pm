...
sub dispatch {
    ...
    's_cdate2',
		...
		's_member_level',
		's_member_st_flag',
		's_member_course_flag',
		's_member_status',
		'limit',
		'offset'
	];
	#入力値を取得
	my $in = $self->get_input_data( $in_names, [ "s_member_purpose", "s_member_demand", "s_member_interest", "s_member_level" ] );
	my $params = {};
	while( my($k, $v) = each %{$in} ) {
		...
		if($k eq "cdate1") {
			if($v =~ /^\d{4}\-\d{2}\-\d{2}$/) {
				my $iso = "${v} 00:00:00";
				$params->{"member_cdate1"} = FCC::Class::Date::Utils->new(time=>$v, tz=>$self->{conf}->{tz}, iso=>$iso)->epoch();
			} else {
				next;
			}
		} elsif($k eq "cdate2") {
			if($v =~ /^\d{4}\-\d{2}\-\d{2}$/) {
				my $iso = "${v} 23:59:59";
				$params->{"member_cdate2"}  = FCC::Class::Date::Utils->new(time=>$v, tz=>$self->{conf}->{tz}, iso=>$iso)->epoch();
			} else {
				next;
			}
		}
	...
	#
	if($params->{member_cdate1}) {
		$res->{params}->{cdate1} = $params->{cdate1};
	}
	if($params->{member_cdate2}) {
