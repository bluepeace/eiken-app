package FCC::Action::Admin::Schlstfrm2Action;
$VERSION = 1.00;
use strict;
use warnings;
...
sub dispatch {
    ...
    #入力値のname属性値のリスト
    my $in_names = [ 's_sch_id', 's_prof_id', 's_lsn_id', 's_sch_cdate', 's_sch_stime_s', 's_sch_stime_e', 'limit', 'offset' ];
    ...
    #レッスン情報（検索条件に講師識別IDが指定された場合）
    if ( defined $res->{params}->{lsn_id} && $res->{params}->{lsn_id} =~ /^\d+$/ ) {
        my $olsn = new FCC::Class::Lesson( conf => $self->{conf}, db => $self->{db} );
        $res->{lsn} = $olsn->get_from_db( $res->{params}->{lsn_id} );
    }

    #CSVダウンロードURL
    {
        my @url_params = ("m=schtsvdwn");
        while ( my ( $k, $v ) = each %{ $res->{params} } ) {
            if ( !defined $v ) { next; }
