...
sub dispatch {
    ...
    #学校種別
    my $member_st_flag = $self->{q}->param("member_st_flag");
    ...
    if ( $lsn_pay_type == 1 ) {    #ポイントの場合
        ...
        if ( $available_point < $course->{course_fee} ) {
            $context->{fatalerrs} = ["レッスン回数が不足しています。"];
            return $context;
        }
        ...
        if ( "$sch->{sch_stime_Y}-$sch->{sch_stime_m}-$sch->{sch_stime_d}" gt $member->{member_point_expire} ) {
            ...
            $m += 0;
            $d += 0;
            $context->{fatalerrs} = ["コース有効期限を過ぎた予約はできません。（あなたのコース有効期限は${y}年${m}月${d}日です）※もし受講回数が残っておりましたら、お問い合わせフォームより運営事務局にご相談ください"];
            return $context;
        }
        ...
        if ( $available_coupon < $course->{course_fee} ) {
            $context->{fatalerrs} = ["体験レッスン有効期限が過ぎているか、もう既に体験レッスンを行っています。"];
            return $context;
        }

        #クーポンの有効期限をチェック
        my $coupon = $ocoupon->get( $member->{coupon_id} );
        if ( "$sch->{sch_stime_Y}-$sch->{sch_stime_m}-$sch->{sch_stime_d}" gt $member->{member_coupon_expire} ) {
            my ( $y, $m, $d ) = split( /\-/, $member->{member_coupon_expire} );
            $y += 0;
            $m += 0;
            $d += 0;
            $context->{fatalerrs} = ["体験レッスン有効期間を超えた日程での予約はできません。（あなたの体験レッスン有効期間は${y}年${m}月${d}日です）"];
            return $context;
        }
    }
