...
sub main {
	...
	for my $auto (@{$candidate_auto_list}) {
		...
		if( ! $auto->{member_id} ) {
			...
			my $sql = "SELECT * FROM members where member_id = ".$auto->{member_id}."";
			my $sth = $dbh->prepare($sql);
			$sth->execute();
			my $stripe_cus_id = "";
			while( my $h  = $sth->fetchrow_hashref ) {
				$stripe_cus_id = $h->{member_stripe_cus_id};
			}

			#PayPalかStripeか判断
            &loging("TIMER", $stripe_cus_id);
			if( $stripe_cus_id ){
				#print "Content-type: text/html\n\n".$c->{payment_type}."\n"; exit(0);

				#PayPalに自動課金をリクエスト
				my $crd_success = stripe_nvp($c, $dbh, $auto, $ocard, $oauto, $ombract, $ot);
				if($crd_success == 1) {
					$success_num ++;
				} else {
					$error_num ++;
...
#			crd_point => $auto->{auto_point},
	...
	#my $me = $dbh->get_from_db($auto->{member_id});
	...
	if( $stripe_cus_id ){
		...
		my $command = 'curl -X POST  https://api.stripe.com/v1/charges -u '.$secret.':'.$secret_pass.' -d amount='.$price.' -d currency=jpy -d customer='.$stripe_cus_id.' -d description="Charge for '.$member_id.' '.$auto->{pln_id}.'"';
