...
sub main {
	...
	my $success_num = 0;
	my $error_num = 0;
	#print scalar @{$candidate_auto_list}, "\n";
	for my $auto (@{$candidate_auto_list}) {
		if( ! $auto->{member_id} ) {
			...
			#Stripeに自動課金をリクエスト
			my $crd_success = stripe_recurring($c, $db, $auto, $ocard, $oauto, $ombract, $ot);
			if($crd_success == 1) {
				$success_num ++;
			} else {
				$error_num ++;
...
sub stripe_recurring {
	my($c, $db, $auto, $ocard, $oauto, $ombract, $ot) = @_;
	#cardsテーブルにレコードを追加
	...
	#StripeのCustomer情報を取得するためのCustomer IDを取得
	my $member_id = $auto->{member_id};
	my $omember = new FCC::Class::Member(conf=>$c, db=>$db);
	my $member = $omember->get_from_db($member_id);
	my $customer_id = $member->{member_stripe_cus_id};


	#APIの認証情報
	my $secret = $c->{stripe_secret_key};
	my $secret_pass = 'password';

	#初回決済の支払い方法を取得
	my $command = 'curl -G https://api.stripe.com/v1/customers/'.$customer_id.'/payment_methods -u '.$secret.':'.$secret_pass;
	my $result = `$command`;
	my $payment_methods = decode_json($result);
	#print "get customers : $result\n";
	if (!$payment_methods) {
		&loging("error", "failed to get payment methods from stripe api: $result");
		return 0;
	}

	my $payment_method_id = '';
	if (ref($payment_methods->{data}) eq 'ARRAY' && @{$payment_methods->{data}} > 0 && exists $payment_methods->{data}[0]{id}) {
		$payment_method_id = $payment_methods->{data}[0]{id};
	} else {
		&loging("error", "failed to get payment methods id from stripe api: $result");
		return 0;
	}

	#決済実行
	my $price = $auto->{pln_price};
	$command = 'curl -X POST https://api.stripe.com/v1/payment_intents -u '.$secret.':'.$secret_pass.' -d amount='.$price.' -d currency=jpy -d customer='.$customer_id.' -d payment_method='.$payment_method_id.' -d description="Charge for '.$member_id.'" -d off_session=true -d confirm=true';

	$result = `$command`;
	#print "get payment_intents : $result\n";
	my $item = decode_json($result);

	my $payment_id =  $item->{id};
	my $status =  $item->{status};

	my $crd_success = 0;
	my $crd_nvp_message = $result;

	if (!$status || $status ne 'succeeded') {
		&loging("error", "failed to payment intents methods from stripe api: $result");
		$crd_success = 2;
	} else {
		$crd_success = 1;
	}
	...
	if($crd_success == 1) {
		...
		$auto_updates->{auto_txn_id} = $payment_id;
	} else {
