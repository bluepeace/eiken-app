...
sub get_status_undetermined_lesson_list {
	...
	#リミット日時から7日前のレッスンまでが対象
#	my $stime_limit_epoch = $etime_limit_epoch - (86400 * 30);
#	my @stm = FCC::Class::Date::Utils->new(time=>$stime_limit_epoch, tz=>$c->{tz})->get(1);
#	my $stime_limit = "$stm[0]-$stm[1]-$stm[2] 00:00:00";
	#
#	my $sql = "SELECT * FROM lessons WHERE lsn_stime>='${stime_limit}' AND lsn_etime<'${etime_limit}' AND lsn_status=0";
...
#	my $stime_limit = "$stm[0]-$stm[1]-$stm[2] 00:00:00";
	#
#	my $sql = "SELECT * FROM lessons WHERE lsn_stime>='${stime_limit}' AND lsn_etime<'${etime_limit}' AND lsn_status>0 AND lsn_charged_date=0";
