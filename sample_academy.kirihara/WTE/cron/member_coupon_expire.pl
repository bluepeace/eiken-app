...
sub main {
	...
	#
	#ポイント失効対象の会員を抽出
	my $member_list = &get_coupon_target_list($c, $dbh);
	#ポイントを失効させる
	my $success_num = 0;
	my $error_num = 0;
	...
	for my $member (@{$member_list}) {
		...
		my $member_coupon = $member->{member_coupon};
		if($member_coupon == 0) { next; }
		#ポイントの売り掛け
		my $member_receivable_coupon = $olsn->get_receivable($member_id, 2);
		my $minus_coupon = $member_coupon - $member_receivable_coupon;
		if($minus_coupon <= 0) { next; }
		#
		my $last_sql;
		eval {
			$last_sql = "UPDATE members SET member_coupon=${member_receivable_coupon} WHERE member_id=${member_id}";
			$dbh->do($last_sql);
			#
			$last_sql = &make_insert_sql("cpnacts", {
				member_id     => $member_id,
				seller_id     => $seller_id,
				coupon_id     => $coupon_id,
				cpnact_type   => 2,
...
sub get_coupon_target_list {
	my($c, $dbh) = @_;
	#今日の日付
	my @tm = FCC::Class::Date::Utils->new(time=>time, tz=>$c->{tz})->get(1);
	my $today = "$tm[0]-$tm[1]-$tm[2]";
	#対象の会員を抽出
	my $sql = "SELECT * FROM members WHERE member_coupon_expire<'${today}' AND member_coupon>0";
