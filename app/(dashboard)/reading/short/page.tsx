...
export default function ReadingShortPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!isProblemTypeEnabledForLevel(level, "short_fill")) {
      setError("この級では短文の語句空所補充は出題されません。");
      return;
    }
    setError(null);
    ...
    <main className="min-h-[calc(100vh-64px)] px-4 py-8 pb-20">
      ...
      <div className="mx-auto max-w-xl space-y-6 rounded-2xl border border-slate-200 bg-white p-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/reading" className="text-sm text-slate-500 hover:underline">
            終了
          </Link>
        </div>

        <div className="min-h-[4rem] whitespace-pre-wrap text-slate-800">
          {formatConversationLines(parts[0])}
          <span className="mx-1 inline-block min-w-[120px] rounded border border-dashed border-slate-400 bg-slate-50 px-2 py-1 text-sm">
            {showResult
