  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-sm text-slate-400">
        初回ログイン時のオンボーディングでユーザーが選べる「一緒に学習するバディ」を登録します。犬・猫・羊など、種類（kind）で分けて追加できます。
      </p>
      ...
      <section className="rounded-xl border border-slate-700 bg-slate-900/50 p-6">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">種類</label>
            <select
              value={newKind}
              onChange={(e) => setNewKind(e.target.value)}
              className="rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-100 focus:border-brand-500 focus:outline-none"
            >
              {KIND_OPTIONS.map((o) => (
            ...
            {list.map((b) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500">
                  {KIND_OPTIONS.find((o) => o.value === b.kind)?.label ?? b.kind}
                </p>
                <button
                  type="button"
